...
train = pd.read_csv('data/trainagri5000.txt', sep='\t', quoting=csv.QUOTE_NONE,
...
nb = MultinomialNB()
nb = nb.fit(train_data_features, train["Category"])
...
test = pd.read_csv('data/testagri5000.txt', sep='\t', quoting=csv.QUOTE_NONE,
                            names=["Category", "Tweet"])
...
nbresult = nb.predict(test_data_features)
...
measure_performance(test_data_features,test["Category"],nb)
